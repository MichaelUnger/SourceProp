...
namespace prop {
  ...
  PropMatrices::PrimaryMap PropMatrices::LoadInterpMatrixMZ0(double M, double Z0) {

    double LgEmin = 12., LgEmax = 22.;
    double MaxDistance = 4.2122656250e+00 * utl::Gpc; 
    int intM = 100*M, intZ0 = 100*Z0;
    std::string strM = std::to_string(abs(intM)), strZ0 = std::to_string(intZ0);

    TFile* fFile;
    std::string InterpFile = "./crp4/CRPropaG12_eEvoM";

    if( M < 0. ) InterpFile += "m" + strM + "z" + strZ0 + "_nu.root";
    else InterpFile += "p" + strM + "z" + strZ0 + "_nu.root";
  ...
  void PropMatrices::UpdateMZ0(double oldM, double newM, double oldZ0, double newZ0) {
    ...
    if(oldM == newM && oldZ0 == newZ0) return;

    int DeltaM = floor(newM/dM)-floor(oldM/dM), DeltaZ0 = floor(newZ0/dZ0)-floor(oldZ0/dZ0); 

    double xL = dM*floor(newM/dM);
    double xR = xL + dM;
    double yD = dZ0*floor(newZ0/dZ0);
    double yU = yD + dZ0;
  ...
  void PropMatrices::InterpInitMZ0(double M, double Z0) {
    ...
    double x = M, y = Z0;
    double xL = dM*floor(M/dM);
    double xR = xL + dM;
    double yD = dZ0*floor(Z0/dZ0);
    double yU = yD + dZ0;
  ...
  void PropMatrices::InterpMZ0(double x, double xL, double xR, double y, double yD, double yU) {

    ResetPrimaryMap();
    std::map<int, int> primkeyMap, seckeyMap;
    for ( auto const& iter1 : InterpMatrixUL) primkeyMap[iter1.first] = 0;
    for ( auto const& iter1 : InterpMatrixUR) primkeyMap[iter1.first] = 0; 
    for ( auto const& iter1 : InterpMatrixDL) primkeyMap[iter1.first] = 0;
    for ( auto const& iter1 : InterpMatrixDR) primkeyMap[iter1.first] = 0; 
    for ( auto const& iter1 : primkeyMap) {
      const unsigned int Aprim = iter1.first;
      seckeyMap.clear();
      for ( auto const& iter2 : InterpMatrixUL[Aprim]) seckeyMap[iter2.first] = 0;
      for ( auto const& iter2 : InterpMatrixUR[Aprim]) seckeyMap[iter2.first] = 0;
      for ( auto const& iter2 : InterpMatrixDL[Aprim]) seckeyMap[iter2.first] = 0;
      for ( auto const& iter2 : InterpMatrixDR[Aprim]) seckeyMap[iter2.first] = 0;
	...
	int maxNcols = max(mUL.GetNcols(), max(mUR.GetNcols(), max(mDL.GetNcols(), mDR.GetNcols())));
	if( (maxNcols != mUL.GetNcols() && mUL.GetNcols() > 0) || (maxNcols != mUR.GetNcols() && mUR.GetNcols() > 0) || (maxNcols != mDL.GetNcols() && mDL.GetNcols() > 0) || (maxNcols != mDR.GetNcols()  && mDR.GetNcols() > 0) )
	  throw runtime_error("PropMatrices column dimension mismatch!");
	int maxNrows = max(mUL.GetNrows(), max(mUR.GetNrows(), max(mDL.GetNrows(), mDR.GetNrows())));
	if( (maxNrows != mUL.GetNrows() && mUL.GetNrows() > 0) || (maxNrows != mUR.GetNrows() && mUR.GetNrows() > 0) || (maxNrows != mDL.GetNrows() && mDL.GetNrows() > 0) || (maxNrows != mDR.GetNrows()  && mDR.GetNrows() > 0) )
...
//			       h->GetXaxis()->GetXmax());
	for (int iPrim = 0; iPrim < m.GetNcols(); ++iPrim) {
		...
		auto valUL = (mUL.GetNcols() == 0 || mUL.GetNrows() == 0)? 0. : mUL[jSec][iPrim];
		auto valUR = (mUR.GetNcols() == 0 || mUR.GetNrows() == 0)? 0. : mUR[jSec][iPrim];
		auto valDL = (mDL.GetNcols() == 0 || mDL.GetNrows() == 0)? 0. : mDL[jSec][iPrim];
		auto valDR = (mDR.GetNcols() == 0 || mDR.GetNrows() == 0)? 0. : mDR[jSec][iPrim];
		...
		double a00, a02, a20, a22, a30, a03, a23, a32, a33, X, Y;
		X = (x-xL)/(xR-xL); Y = (y-yD)/(yU-yD);
		a00 = valDL; a22 = 9.*(valDL+valUR-valUL-valDR); a33 = 4.*(valDL+valUR-valUL-valDR);
		a02 = 3.*(valUL-valDL); a20 = 3.*(valDR-valDL);
		a03 = 2.*(valDL-valUL); a30 = 2.*(valDL-valDR);
		a23 = 6.*(valDR+valUL-valDL-valUR); a32 = a23;
		m[jSec][iPrim] = a00 + a02*pow(Y, 2) + a20*pow(X, 2) + a03*pow(Y, 3) + a30*pow(X, 3) + a22*pow(X*Y, 2) + a33*pow(X*Y, 3) + a23*pow(X, 2)*pow(Y, 3) + a32*pow(X, 3)*pow(Y, 2);
	   }
	}
       }
     }
  ...
  PropMatrices::PrimaryMap PropMatrices::LoadInterpMatrixDmin(double Dmin) {
  ...
  void PropMatrices::UpdateDmin(double oldDmin, double newDmin) {

    if( newDmin > DminMax || newDmin < DminMin )  
	throw runtime_error("Dmin drifted outside range: Dmin = " + std::to_string(newDmin) + " Mpc");

    // if nothing has changed don't update
    if(oldDmin == newDmin) return;

    int newposR = (upper_bound(DminGrid.begin(), DminGrid.end(), newDmin) - DminGrid.begin());
    int deltapos = newposR - posR;
    double xL = DminGrid[newposR-1], xR = DminGrid[newposR];
  ...
  void PropMatrices::InterpInitDmin(double Dmin) {
    ...
    double x = Dmin, xL = DminGrid[posR-1], xR = DminGrid[posR];
  ...
  void PropMatrices::InterpDmin(double x, double xL, double xR) {
 
    ResetPrimaryMap();
    std::map<int, int> primkeyMap, seckeyMap;
    for ( auto const& iter1 : InterpMatrixL) primkeyMap[iter1.first] = 0;
    for ( auto const& iter1 : InterpMatrixR) primkeyMap[iter1.first] = 0; 
    for ( auto const& iter1 : primkeyMap) {
      const unsigned int Aprim = iter1.first;
      seckeyMap.clear();
      for ( auto const& iter2 : InterpMatrixL[Aprim]) seckeyMap[iter2.first] = 0;
      for ( auto const& iter2 : InterpMatrixR[Aprim]) seckeyMap[iter2.first] = 0;
      for ( auto const& iter2 : seckeyMap) {
	const unsigned int Asec = iter2.first;
	...
	m.ResizeTo(mL.GetNrows(), mL.GetNcols());
	    for (int iPrim = 0; iPrim < m.GetNcols(); ++iPrim) {
		for (int jSec = 0; jSec < m.GetNrows(); ++jSec) {
		  auto valL = (mL.GetNcols() == 0 || mL.GetNrows() == 0)? 0. : mL[jSec][iPrim];
		  auto valR = (mR.GetNcols() == 0 || mR.GetNrows() == 0)? 0. : mR[jSec][iPrim];
		  ...
		  B = 3.*(valR-valL);
		  C = valL;
		  m[jSec][iPrim] = A*pow(X, 3) + B*pow(X, 2) + C;
		}
	      }
      }
    }
