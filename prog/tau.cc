...
const double kSigmaT = 6.6524587158e-29*m2;
...
SigmaGammaGamma(const double gammaCM2)
{
  if (gammaCM2 < 1)
    ...
    return (log(2*gammaCM2) - 1) / gammaCM2 *  kSigmaT * 3/8;
  ...
  return 3/16.* kSigmaT * (1 - betaCM2) * (t1 - t2);
}
...
main()
{
  ...
  TH1D hBB("hBB", "; lg(E/eV); dn/dE [m^{-3} eV^{-1};", nX, x1, x2);
  TH1D hSigma("hSigma", "; E [eV]; #sigma_{e^{+}e^{-}}/(#pi r_e^{2});",
              100, -0.5, 3);
  TH1D hLambda("hLambda", "; lg(E/eV); #lambda} [Mpc];", nY, y1, y2);

  const double T = 10*kelvin;
  ...
  for (int i = 0; i < hSigma.GetNbinsX(); ++i) {
    ...
    const double piRe2 = kSigmaT * 3/8.;
  ...
  for (unsigned int i = 0; i < nX; ++i) {
    ...
    hBB.SetBinContent(i+1, bb / (1/cm3/eV));
  }
  ...
  for (unsigned int i = 0; i < nY; ++i) {
    ...
    for (unsigned int j = 0; j < nX; ++j) {
      ...
      const double nextLgEtarget = x1 + (j+1)*dx + dx/2;
      const double nextTargetEnergy = pow(10, nextLgEtarget) * eV;
      const double dE = nextTargetEnergy - targetEnergy;
      sumTarget += (cosSum*dx*n*dE);
    }
    if (sumTarget > 1/(1000*Gpc))
      hLambda.SetBinContent(i+1, 1./sumTarget/Mpc);
  }
